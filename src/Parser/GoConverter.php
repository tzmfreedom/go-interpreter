...
namespace GoInterpreter\Parser;

use GoInterpreter\Parser\Ast\Arguments;
...
class GoConverter extends GoParserBaseVisitor
{
	...
	{
        $expr = $context->expressionList()->accept($this);
	    return new Return_($expr[0]);
	}
	...
	public function visitExpression(Context\ExpressionContext $context)
	{
        ...
        if ($context->mul_op) {
            $left = $context->expression(0)->accept($this);
            $right = $context->expression(1)->accept($this);
            return new BinaryExpression($left, $right, $context->mul_op->getText());
        }
        if ($context->add_op) {
            $left = $context->expression(0)->accept($this);
            $right = $context->expression(1)->accept($this);
            return new BinaryExpression($left, $right, $context->add_op->getText());
        }
        if ($context->rel_op) {
            $left = $context->expression(0)->accept($this);
            $right = $context->expression(1)->accept($this);
            return new BinaryExpression($left, $right, $context->rel_op->getText());
        }
        if ($context->LOGICAL_OR()) {
            $left = $context->expression(0)->accept($this);
            $right = $context->expression(1)->accept($this);
            return new BinaryExpression($left, $right, $context->LOGICAL_OR()->getText());
        }
        if ($context->LOGICAL_AND()) {
            $left = $context->expression(0)->accept($this);
            $right = $context->expression(1)->accept($this);
            return new BinaryExpression($left, $right, $context->LOGICAL_AND()->getText());
        }
        return null;
	}
	...
	public function visitInteger(Context\IntegerContext $context)
	{
	    if ($context->DECIMAL_LIT()) {
	        return new Integer((int)$context->DECIMAL_LIT()->getText());
        }
	    return null;
	}
